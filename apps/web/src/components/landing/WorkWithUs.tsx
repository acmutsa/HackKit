...
// Polaroid signature:
function Polaroid({ person, slotIndex }: { person: Person; slotIndex: number }) {
	...
	return (
		<div
			className="h-auto w-[40vw] sm:w-[25vw] md:w-[21vw] lg:w-[16vw] xl:w-[16vw] rounded-[3px] bg-[#F0EDE7] px-[5%] pt-[12%] pb-[15%] sm:pb-[18%] md:pb-[20%] lg:pb-[23%] xl:pb-[22%] rounded-[3px] z-40 shadow-[0_7px_10px_rgba(0,0,0,0.28)] drop-shadow-[2px_5px_1px_rgba(0,0,0,0.45)] md:w-[15vw]"
		...
		>
			...
			<div className=" w-[100%] h-auto pt-[5%] pb-[10%] px-[5%] relative flex items-center justify-center gap-2 leading-none">
				{/* name crossfade */}
				<div className="relative w-[100%] h-fit">
					<p
						style={{ transition: "opacity 2000ms ease-in-out" }}
						className={`w-[100%] h-auto sm:text-base md:text-lg lg:text-xl xl:text-3xl 2xl:text-4xl ${shadowsIntoLight.className} ${showFront ? "opacity-100" : "opacity-0"}`}
					>
						{front.fname} {front.lname}
					</p>
					<p
						style={{ transition: "opacity 2000ms ease-in-out" }}
						className={`w-[100%] h-auto absolute inset-0 sm:text-base md:text-lg lg:text-xl xl:text-3xl 2xl:text-4xl ${shadowsIntoLight.className} ${showFront ? "opacity-0" : "opacity-100"}`}
					>
				...
				<div className="relative h-4 sm:h-5 md:h-6 lg:h-8 xl:h-10 w-4 sm:w-5 md:w-6 lg:w-8 xl:w-10 ">
					{front.linkedin && (
							<Link
								href={front.linkedin}
							...
							<Linkedin
								style={{ transition: "opacity 2000ms ease-in-out" }}
								className={`h-4 sm:h-5 md:h-6 lg:h-8 xl:h-10 w-4 sm:w-5 md:w-6 lg:w-8 xl:w-10  p-[15%] invert dark:invert-0 ${showFront ? "opacity-100" : "opacity-0"}`}
							/>
						</Link>
					)}
					{back.linkedin && (
							...
							<Linkedin
								style={{ transition: "opacity 2000ms ease-in-out" }}
								className={`h-4 sm:h-5 md:h-6 lg:h-8 xl:h-10 w-4 sm:w-5 md:w-6 lg:w-8 xl:w-10 p-[15%] invert dark:invert-0 ${showFront ? "opacity-0" : "opacity-100"}`}
							/>
						</Link>
					)}
				</div>
		...
		{(() => {
			...
			return (
				<div className="absolute rotate-[-10deg]" style={{ top: active.top, left: active.left }}>

						
							<p
								key={`${slotIndex}-${active.note}`} // remount → re-animate whenever the note changes
								className={`w-full text-xl sm:text-xl md:text-2xl lg:text-3xl xl:text-5xl text-[#AC1903] ${shadowsIntoLight.className}`}
							>
								{active.note.split("").map((char, i) => (
									<motion.span
										...
										transition={{ delay: 2 + i * 0.05, duration: 1 }}
...
export default function WorkWithUs() {
	...
	return (
		<section
			className="relative w-full flex items-center justify-center pb-[12vw] sm:pb-[3vw] md:pb-[5vw]"
			id="WorkWithUs"
		>
			
			<div className="w-full px-[3vw]">
				<div className="relative flex flex-col items-center justify-center gap-y-6 pb-[3%] sm:pb-[8%] pl-[20%]">
					<div
						className={`relative h-fit w-[48vw] sm:w-[38vw] z-40 bg-contain bg-center bg-no-repeat ${shadowsIntoLight.className} rotate-[13deg] drop-shadow-[2px_5px_1px_rgba(0,0,0,0.45)]`}
						style={{
							backgroundImage:
				...
				<div className="flex items-center justify-center gap-[3vw] md:gap-[1.6vw]">
					...
					{slots.map((slot) => {
						...
						const visibility = slot < 2 ? "flex" : slot === 2 ? "hidden sm:flex" : slot === 3 ? "hidden md:flex" : "hidden lg:flex"; 
						return (
							<div key={slot} className={visibility}>
								<Polaroid person={person} slotIndex={person_id} />
							</div>
						);
					})}
