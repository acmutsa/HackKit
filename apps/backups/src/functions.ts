...
async function getDatabaseDump(databseName:string, organizationSlug:string){
  const res = await fetch(`https://${databseName}-${organizationSlug}.turso.io/dump`,{
    method:"GET",
    headers:new Headers({
  ...
  if (!res.ok) {
    throw new Error(`Failed to get database dump: ${res.status} ${res.statusText}`);
  }
...
async function doBackup(){
  const backupData = await getDatabaseDump(env.BACKUPS_DATABSE_NAME, env.BACKUPS_ORGANIZATION_SLUG);
