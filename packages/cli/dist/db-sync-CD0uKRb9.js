...
//#endregion
//#region ../core/dist/adapters/db/drizzle.js
function toDrizzleTableName(modelKey) {
	return modelKey.replaceAll(".", "_");
}
function toTableName(modelKey) {
...
function buildCreateTableStatement(model) {
...
function formatSqlDefault(field) {
	if (!field.defaultValue || field.defaultValue.kind !== "static") return null;
	const value = field.defaultValue.value;
	if (field.kind === "boolean") return value ? "1" : "0";
	if (field.kind === "string" || field.kind === "enum" || field.kind === "json") return `'${String(value).replaceAll("'", "''")}'`;
	if (typeof value === "number") return String(value);
	return null;
}
function buildAddColumnStatement(tableName, columnName, field) {
	...
	if (!field.isOptional && defaultSql !== null) column += " NOT NULL";
	return `ALTER TABLE ${quoteIdentifier(tableName)} ADD COLUMN ${column}`;
}
async function getExistingColumns(db, tableName) {
	const rows = await db.all(sql.raw(`PRAGMA table_info(${quoteIdentifier(tableName)})`));
	return new Set(rows.map((row) => row.name));
}
async function syncDrizzleStorage(db, storage) {
	for (const model of Object.values(storage.models)) {
		...
		for (const [columnName, field] of Object.entries(model.schema.fields)) {
			...
			await db.run(sql.raw(buildAddColumnStatement(tableName, columnName, field)));
		}
	}
}
//#endregion
//#region ../core/dist/database.js
function createField(definition) {
	const withDefinition = (next) => createField(next);
	return {
		...definition,
		optional() {
...
const field = {
	...
	enum: (values) => createField({
		kind: "enum",
		isOptional: false,
		enumValues: values
	})
};
function defineModel(key, schema) {
	return {
		key,
		schema
	};
...
ZodPipeline.create;
//#endregion
//#region ../core/dist/errors.js
var HackKitError = class HackKitError extends Error {
	code;
	cause;
	details;
...
//#endregion
//#region ../core/dist/permissions.js
...
const coreModels = {
	user: defineModel("core.user", {
		fields: {
			...
			roleId: field.string().optional().references("core.role", "id", { onDelete: "setNull" }),
	...
	userData: defineModel("core.userData", { fields: {
		authId: field.string().primaryKey().references("core.user", "authId", { onDelete: "cascade" }),
		age: field.integer(),
		gender: field.string(),
		race: field.string(),
		ethnicity: field.string(),
		shirtSize: field.string(),
		dietaryRestrictions: field.json().default([]),
	...
	role: defineModel("core.role", {
		fields: {
			...
			permissions: field.json().default([]),
	...
	userBan: defineModel("core.userBan", { fields: {
		authId: field.string().primaryKey().references("core.user", "authId", { onDelete: "cascade" }),
	...
	eventScan: defineModel("core.eventScan", {
		fields: {
			id: field.string().primaryKey().defaultId(),
			eventId: field.string().references("core.event", "id", { onDelete: "cascade" }),
			authId: field.string().references("core.user", "authId", { onDelete: "cascade" }),
...
function createPluginRegistry(plugins = []) {
	...
	for (const plugin of plugins) {
		if (registry.plugins[plugin.id]) throw new HackKitError("CONFLICT", `HackKit plugin '${plugin.id}' is already registered.`);
		...
		for (const [name, pluginModel] of Object.entries(plugin.models ?? {})) {
			...
			if (!modelKey.startsWith(`${plugin.id}.`)) throw new HackKitError("INVALID_OPERATION", `Plugin model '${name}' must use the '${plugin.id}.' namespace.`);
			if (registry.models[modelKey]) throw new HackKitError("CONFLICT", `HackKit model '${modelKey}' is already registered.`);
			registry.models[modelKey] = pluginModel;
			registry.storage.models[modelKey] = pluginModel;
		}
		for (const [name, permission] of Object.entries(plugin.permissions ?? {})) {
			if (!permission.startsWith(`${plugin.id}.`)) throw new HackKitError("INVALID_OPERATION", `Plugin permission '${name}' must use the '${plugin.id}.' namespace.`);
			if (Object.values(registry.permissions).includes(permission)) throw new HackKitError("CONFLICT", `HackKit permission '${permission}' is already registered.`);
...
//#endregion
//#region ../core/dist/user-data-options.js
const defaultUserDataOptions = {
	gender: [
		{
			value: "woman",
			label: "Woman"
...
const userDataOptionSchema = objectType({
	value: stringType().min(1),
	label: stringType().min(1)
});
objectType({
	gender: arrayType(userDataOptionSchema).min(1),
	race: arrayType(userDataOptionSchema).min(1),
	ethnicity: arrayType(userDataOptionSchema).min(1),
	shirtSize: arrayType(userDataOptionSchema).min(1),
	dietaryRestrictions: arrayType(userDataOptionSchema).min(1),
	countryOfResidence: arrayType(userDataOptionSchema).min(1)
});
function optionValueSchema(options, message) {
	const values = new Set(options.map((option) => option.value));
	return stringType().min(1).refine((value) => values.has(value), { message });
}
function createCompleteUserDataSchema(options) {
	const dietaryValues = new Set(options.dietaryRestrictions.map((option) => option.value));
	return objectType({
		authId: stringType().min(1),
		age: numberType().int().nonnegative(),
		gender: optionValueSchema(options.gender, "Select a valid gender."),
		race: optionValueSchema(options.race, "Select a valid race."),
		ethnicity: optionValueSchema(options.ethnicity, "Select a valid ethnicity."),
		shirtSize: optionValueSchema(options.shirtSize, "Select a valid shirt size."),
		dietaryRestrictions: arrayType(stringType().refine((value) => dietaryValues.has(value), { message: "Select a valid dietary restriction." })).default([]),
		accommodationNote: stringType().optional(),
		phoneNumber: stringType().optional(),
		countryOfResidence: optionValueSchema(options.countryOfResidence, "Select a valid country of residence.").optional(),
		hasAcceptedMLHCodeOfConduct: booleanType(),
		hasSharedDataWithMLH: booleanType(),
		isEmailable: booleanType()
	});
}
//#endregion
//#region ../core/dist/schemas.js
const permissionKeySchema = stringType().regex(/^[a-z0-9_-]+\.[a-z0-9_.-]+$/);
const authIdSchema = stringType().min(1);
const roleIdSchema = stringType().min(1);
const hackTagSchema = stringType().min(1).max(50).regex(/^[a-zA-Z0-9_-]+$/).transform((value) => value.toLowerCase());
objectType({
	authId: authIdSchema,
	email: stringType().email(),
	firstName: stringType().min(1).max(100),
	lastName: stringType().min(1).max(100),
	profilePhotoUrl: stringType().url().optional()
});
objectType({
	authId: authIdSchema,
	hackTag: hackTagSchema
});
createCompleteUserDataSchema(defaultUserDataOptions);
objectType({
	authId: authIdSchema,
	university: stringType().min(1),
	major: stringType().min(1),
	schoolId: stringType().optional(),
	levelOfStudy: stringType().min(1),
	hackathonsAttended: numberType().int().nonnegative(),
	softwareExperience: stringType().min(1),
	heardFrom: stringType().optional(),
	githubUrl: stringType().url().optional(),
	linkedInUrl: stringType().url().optional(),
	personalWebsiteUrl: stringType().url().optional(),
	resumeUrl: stringType().url().optional(),
	group: stringType().optional()
});
const actorSchema = objectType({ actorAuthId: authIdSchema });
actorSchema.extend({
	id: roleIdSchema.optional(),
	name: stringType().min(1).max(50),
	position: numberType().int().nonnegative(),
	permissions: arrayType(permissionKeySchema),
	color: stringType().optional()
});
actorSchema.extend({
	roleId: roleIdSchema,
	name: stringType().min(1).max(50).optional(),
	position: numberType().int().nonnegative().optional(),
	permissions: arrayType(permissionKeySchema).optional(),
	color: stringType().optional()
});
actorSchema.extend({ roleId: roleIdSchema });
actorSchema.extend({
	targetAuthId: authIdSchema,
	roleId: roleIdSchema
});
objectType({ authId: authIdSchema });
actorSchema.extend({
	targetAuthId: authIdSchema,
...
actorSchema.extend({ targetAuthId: authIdSchema });
actorSchema.extend({ targetAuthId: authIdSchema });
actorSchema.extend({ targetAuthId: authIdSchema });
actorSchema.extend({ eventId: stringType().min(1) });
objectType({
	eventId: stringType().min(1),
...
//#endregion
//#region src/db-sync.ts
var db_sync_exports = /* @__PURE__ */ __exportAll({ runDbSync: () => runDbSync });
async function runDbSync(config) {
	const db = drizzle(createClient({ url: config.databaseUrl }));
	await syncDrizzleStorage(db, createPluginRegistry(config.plugins ?? []).storage);
	if (config.auth?.syncStorage) await config.auth.syncStorage(db);
	console.log("HackKit database sync completed.");
}
//#endregion
export { runDbSync as n, db_sync_exports as t };

//# sourceMappingURL=db-sync-CD0uKRb9.js.map